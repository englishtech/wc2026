...
import re
import pandas as pd
from config import logger, ELORATINGS_URL_1, ELORATINGS_URL_2, WC2026_QUALIFIERS_FILE
...
def clean_eloratings():
    ...
    df.to_csv(f"{WC2026_QUALIFIERS_FILE}_clean.csv", index=False, encoding="utf-8")
    print(f"Готово: {len(df)} матчей")  # Выводим итог


'''
parse_eloratings(ELORATINGS_URL_1)
clean_eloratings()
'''